...
import { Shield, Loader2, TrendingUp, RefreshCw, ExternalLink, Check, Lock } from "lucide-react";
  ...
  const getQuote = async () => {
    ...
    try {
      const res = await axios.post(`${API}/uniswap/quote`, {
        chain, token_in: tokenIn, token_out: tokenOut,
        amount_in: amount, stealth_recipient: stealthRecipient, fee_tier: feeTier
      });
      setQuote(res.data);
  ...
  const executeSwap = async () => {
    ...
    try {
      ...
      await axios.post(`${API}/uniswap/record-swap`, {
        tx_hash: tx.hash, from_address: address, token_in: tokenIn, token_out: tokenOut,
        amount_in: amount, amount_out: quote.amount_out_human, chain,
        stealth_recipient: stealthRecipient, router_used: "uniswap_v3"
      });
    ...
    <div className="space-y-4" data-testid="uniswap-private-swap">
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="bg-white/5 border border-white/20 p-4">
          ...
          <select value={tokenIn} onChange={e => setTokenIn(e.target.value)} data-testid="uniswap-token-in"
        ...
        <div className="bg-white/5 border border-white/20 p-4">
          ...
          <select value={tokenOut} onChange={e => setTokenOut(e.target.value)} data-testid="uniswap-token-out"
            className="w-full bg-transparent text-base font-semibold outline-none">
      ...
      <div className="space-y-2">
        ...
        <input data-testid="uniswap-stealth-input" value={stealthRecipient}
          onChange={e => setStealthRecipient(e.target.value)} placeholder="0x... (stealth address)"
      ...
      <div className="flex items-center gap-3">
        ...
        <div className="text-xs text-gray-500">
          ...
          {quote?.quote_source && <div className="text-white/30">Source: {quote.quote_source}</div>}
        </div>
      </div>

...
import { Shield, TrendingUp, Check, Loader2 } from "lucide-react";
...
export function HyperliquidPrivateTrading() {
  ...
  const prepareTrade = async () => {
    ...
    setPreparing(true); setTradePlan(null);
    try {
  ...
  return (
    <div className="space-y-4" data-testid="hyperliquid-trading">
      <div className="bg-green-500/10 border border-green-500/30 p-3 rounded text-xs text-green-300">
        ...
        Your margin is routed through a stealth proxy before opening positions on Hyperliquid. Your wallet is never linked to the trade.
      </div>
      ...
      {tradePlan && (
        <div className="bg-green-500/10 border border-green-500/30 p-4 space-y-3">
          <div className="flex items-center gap-2 text-green-400 font-semibold text-sm"><Check className="w-4 h-4" /> Trade Prepared Successfully</div>

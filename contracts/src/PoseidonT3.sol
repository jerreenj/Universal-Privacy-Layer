...
/*
 * PoseidonT3 — 2-input Poseidon hash over BN254 (circomlib parameters).
 * GENERATED by scripts/gen_poseidon_sol.js from the constants in the VENDORED
 * circomlib/circuits/poseidon_constants.circom — the SAME constants withdraw.circom
 * compiles against — so on-chain hash === in-circuit hash. The optimized circomlib
 * structure: 8 full rounds + 57 sparse partial rounds, width t=3.
 * Attribution: iden3/circomlib (MIT). Not hand-rolled.
 *
 * Verified: scripts/verify_poseidon_ref.js reproduces circomlibjs on
 *   poseidon(0,0), poseidon(1,0), poseidon(1,2) — this Solidity is a mechanical
 *   port of that verified reference. Lane order: state = [0, a, b] (PoseidonEx).
 *
 * Emission note: round constants / matrices are emitted as `pure` functions
 *   returning memory-array literals (NOT `uint256[N] constant`) because solc 0.8.x
...
library PoseidonT3 {
    ...
    /// @dev Round constants C (flat, 81). Layout per circomlib: [0..t-1]=ark0, then
    /// @dev (nRoundsF/2-1) full rounds of t each, then a P-round's t, then nRoundsP single
    /// @dev partial-round constants, then the final (nRoundsF/2-1) full rounds of t each.
    function C() internal pure returns (uint256[81] memory r) {
        r = [
    ...
    /// @dev MDS matrix M row 0 (M[j][i] — col i indexes output i; row j). t*t row-major.
    function M0() internal pure returns (uint256[3] memory r) {
        r = [
    ...
    /// @dev MDS matrix M row 1.
    function M1() internal pure returns (uint256[3] memory r) {
        r = [
    ...
    /// @dev MDS matrix M row 2.
    function M2() internal pure returns (uint256[3] memory r) {
        r = [
    ...
    /// @dev M^(nRoundsP) product matrix P row 0 (boundary full round uses P, not M).
    function P0() internal pure returns (uint256[3] memory r) {
        r = [
            0x109b7f411ba0e4c9b2b70caf5c36a7b194be7c11ad24378bfedb68592ba8118b,
    ...
    /// @dev M^(nRoundsP) product matrix P row 1.
    function P1() internal pure returns (uint256[3] memory r) {
        r = [
            0x16ed41e13bb9c0c66ae119424fddbcbc9314dc9fdbdeea55d6c64543dc4903e0,
    ...
    /// @dev M^(nRoundsP) product matrix P row 2.
    function P2() internal pure returns (uint256[3] memory r) {
        r = [
            0x2b90bba00fca0589f617e7dcbfe82e0df706ab640ceb247b791a93b74e36736d,
    ...
    /// @dev Sparse partial-round constants S (flat, 285). Per round r:
    /// @dev out[0] = sum_i S[r*(2t-1)+i]*in[i]; out[i>0] = in[i] + S[r*(2t-1)+(t-1)+i]*in[0].
    /// @dev For t=3 each round consumes S[r*5+0..4]: [0..2] scale the dot for out[0],
    /// @dev [3] scales in[0] into out[1], [4] scales in[0] into out[2].
    function S() internal pure returns (uint256[285] memory r) {
        r = [
            0x109b7f411ba0e4c9b2b70caf5c36a7b194be7c11ad24378bfedb68592ba8118b,
    ...
    function pow5(uint256 a) internal pure returns (uint256) {
        unchecked {
            uint256 a2 = mulmod(a, a, Q);
            return mulmod(a2, mulmod(a2, a, Q), Q);
        }
    }

    /// @dev circomlib Mix — out[i] = sum_j M[j][i]*in[j] (column i of M dotted
    ///      with the input). M0()/M1()/M2() are M ROWS, so out[i] reads column i
    ///      = [M0[i], M1[i], M2[i]]. Verified vs scripts/verify_poseidon_ref.js.
    function mixFull(uint256 a, uint256 b, uint256 c, uint256[3] memory m0, uint256[3] memory m1, uint256[3] memory m2)
        internal
        pure
        returns (uint256 s0, uint256 s1, uint256 s2)
    {
        unchecked {
            // out[i] = in[0]*M[0][i] + in[1]*M[1][i] + in[2]*M[2][i]  (column i).
            s0 = addmod(addmod(mulmod(a, m0[0], Q), mulmod(b, m1[0], Q), Q), mulmod(c, m2[0], Q), Q);
            s1 = addmod(addmod(mulmod(a, m0[1], Q), mulmod(b, m1[1], Q), Q), mulmod(c, m2[1], Q), Q);
            s2 = addmod(addmod(mulmod(a, m0[2], Q), mulmod(b, m1[2], Q), Q), mulmod(c, m2[2], Q), Q);
        }
    }

    /// @dev circomlib MixS — sparse partial-round mix. base = r*(2t-1)=r*5.
    ///      o0 = a*s0+b*s1+c*s2; o1 = b + a*s3; o2 = c + a*s4 (s0..s4 = S[base..base+4]).
    function mixS(uint256 a, uint256 b, uint256 c, uint256 s0, uint256 s1, uint256 s2, uint256 s3, uint256 s4)
        internal
        pure
        returns (uint256 o0, uint256 o1, uint256 o2)
    {
        unchecked {
            o0 = addmod(addmod(mulmod(a, s0, Q), mulmod(b, s1, Q), Q), mulmod(c, s2, Q), Q);
            o1 = addmod(b, mulmod(a, s3, Q), Q);
            o2 = addmod(c, mulmod(a, s4, Q), Q);
        }
    }

    /// @dev 2-input Poseidon. Returns poseidon(a, b) in the BN254 scalar field.
    ...
    function poseidon(uint256 a, uint256 b) internal pure returns (uint256) {
        unchecked {
            ...
            uint256 s1 = a;
            uint256 s2 = b;
            // Load constants once per call (memory literals — see emission note).
            uint256[81] memory _c = C();
            uint256[285] memory _s = S();
            uint256[3] memory _m0 = M0();
            uint256[3] memory _m1 = M1();
            uint256[3] memory _m2 = M2();
            uint256[3] memory _p0 = P0();
            uint256[3] memory _p1 = P1();
            uint256[3] memory _p2 = P2();
            // ark[0]: s_i += C[i].
            s0 = addmod(s0, _c[0], Q);
            s1 = addmod(s1, _c[1], Q);
            s2 = addmod(s2, _c[2], Q);
            // full round r=0 (sigma -> ark@3 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[3], Q);
            s1 = addmod(s1, _c[4], Q);
            s2 = addmod(s2, _c[5], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // full round r=1 (sigma -> ark@6 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[6], Q);
            ...
            s2 = addmod(s2, _c[8], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // full round r=2 (sigma -> ark@9 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[9], Q);
            s1 = addmod(s1, _c[10], Q);
            s2 = addmod(s2, _c[11], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // P-boundary full round (sigma -> ark@12 -> mix P)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[12], Q);
            s1 = addmod(s1, _c[13], Q);
            s2 = addmod(s2, _c[14], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _p0, _p1, _p2);
            // 57 sparse partial rounds (sigma s0 -> +C@[15+r] -> mixS)
            s0 = pow5(s0);
            s0 = addmod(s0, _c[15], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[0], _s[1], _s[2], _s[3], _s[4]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[16], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[5], _s[6], _s[7], _s[8], _s[9]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[17], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[10], _s[11], _s[12], _s[13], _s[14]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[18], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[15], _s[16], _s[17], _s[18], _s[19]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[19], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[20], _s[21], _s[22], _s[23], _s[24]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[20], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[25], _s[26], _s[27], _s[28], _s[29]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[21], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[30], _s[31], _s[32], _s[33], _s[34]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[22], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[35], _s[36], _s[37], _s[38], _s[39]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[23], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[40], _s[41], _s[42], _s[43], _s[44]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[24], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[45], _s[46], _s[47], _s[48], _s[49]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[25], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[50], _s[51], _s[52], _s[53], _s[54]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[26], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[55], _s[56], _s[57], _s[58], _s[59]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[27], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[60], _s[61], _s[62], _s[63], _s[64]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[28], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[65], _s[66], _s[67], _s[68], _s[69]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[29], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[70], _s[71], _s[72], _s[73], _s[74]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[30], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[75], _s[76], _s[77], _s[78], _s[79]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[31], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[80], _s[81], _s[82], _s[83], _s[84]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[32], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[85], _s[86], _s[87], _s[88], _s[89]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[33], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[90], _s[91], _s[92], _s[93], _s[94]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[34], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[95], _s[96], _s[97], _s[98], _s[99]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[35], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[100], _s[101], _s[102], _s[103], _s[104]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[36], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[105], _s[106], _s[107], _s[108], _s[109]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[37], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[110], _s[111], _s[112], _s[113], _s[114]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[38], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[115], _s[116], _s[117], _s[118], _s[119]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[39], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[120], _s[121], _s[122], _s[123], _s[124]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[40], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[125], _s[126], _s[127], _s[128], _s[129]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[41], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[130], _s[131], _s[132], _s[133], _s[134]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[42], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[135], _s[136], _s[137], _s[138], _s[139]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[43], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[140], _s[141], _s[142], _s[143], _s[144]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[44], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[145], _s[146], _s[147], _s[148], _s[149]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[45], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[150], _s[151], _s[152], _s[153], _s[154]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[46], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[155], _s[156], _s[157], _s[158], _s[159]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[47], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[160], _s[161], _s[162], _s[163], _s[164]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[48], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[165], _s[166], _s[167], _s[168], _s[169]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[49], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[170], _s[171], _s[172], _s[173], _s[174]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[50], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[175], _s[176], _s[177], _s[178], _s[179]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[51], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[180], _s[181], _s[182], _s[183], _s[184]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[52], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[185], _s[186], _s[187], _s[188], _s[189]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[53], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[190], _s[191], _s[192], _s[193], _s[194]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[54], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[195], _s[196], _s[197], _s[198], _s[199]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[55], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[200], _s[201], _s[202], _s[203], _s[204]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[56], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[205], _s[206], _s[207], _s[208], _s[209]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[57], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[210], _s[211], _s[212], _s[213], _s[214]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[58], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[215], _s[216], _s[217], _s[218], _s[219]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[59], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[220], _s[221], _s[222], _s[223], _s[224]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[60], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[225], _s[226], _s[227], _s[228], _s[229]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[61], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[230], _s[231], _s[232], _s[233], _s[234]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[62], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[235], _s[236], _s[237], _s[238], _s[239]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[63], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[240], _s[241], _s[242], _s[243], _s[244]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[64], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[245], _s[246], _s[247], _s[248], _s[249]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[65], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[250], _s[251], _s[252], _s[253], _s[254]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[66], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[255], _s[256], _s[257], _s[258], _s[259]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[67], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[260], _s[261], _s[262], _s[263], _s[264]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[68], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[265], _s[266], _s[267], _s[268], _s[269]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[69], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[270], _s[271], _s[272], _s[273], _s[274]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[70], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[275], _s[276], _s[277], _s[278], _s[279]);
            s0 = pow5(s0);
            s0 = addmod(s0, _c[71], Q);
            (s0, s1, s2) = mixS(s0, s1, s2, _s[280], _s[281], _s[282], _s[283], _s[284]);
            // final full round r=0 (sigma -> ark@72 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[72], Q);
            s1 = addmod(s1, _c[73], Q);
            s2 = addmod(s2, _c[74], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // final full round r=1 (sigma -> ark@75 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[75], Q);
            s1 = addmod(s1, _c[76], Q);
            s2 = addmod(s2, _c[77], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // final full round r=2 (sigma -> ark@78 -> mix M)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            s0 = addmod(s0, _c[78], Q);
            s1 = addmod(s1, _c[79], Q);
            s2 = addmod(s2, _c[80], Q);
            (s0, s1, s2) = mixFull(s0, s1, s2, _m0, _m1, _m2);
            // final sigma + mixLast[0] (dot M column 0)
            s0 = pow5(s0);
            s1 = pow5(s1);
            s2 = pow5(s2);
            return addmod(addmod(mulmod(s0, _m0[0], Q), mulmod(s1, _m1[0], Q), Q), mulmod(s2, _m2[0], Q), Q);
        }
    }
}

...
P2.10 Solana (SVM) parity w/ Base+Sui   ██████████████░░  85% 🔨 devnet deploy done (Step 10a); mainnet (10b) gated on SOL
P3  Real ZK (Circom + verifier)     ░░░░░░░░░░░░░░░░   0% ⏸️ not started
P4  Privacy pools + DeFi privacy    ░░░░░░░░░░░░░░░░   0% ⏸️ not started
```

**Last updated:** 2026-07-02 — P2.10 Step 10a DONE: Solana deployed to **DEVNET**
($0 pilot-ready path — full app demonstrable end-to-end without real SOL). Backend
default cluster flipped to devnet (env-overridable); frontend RPC env-driven with an
honest "devnet / test mode" badge; relayer + new `deploy_sol_devnet.sh` target devnet.
Step 10b (mainnet) remains pending SOL funding — a one-command redeploy with the SAME
deployer keypair (same program ID) + flip of 3 config constants. Base + Sui stay mainnet.
...
### Step 10b — Solana on MAINNET ⏸️ pending SOL funding

Same flow against mainnet. Because the deployer keypair at
`target/deploy/upl_sol-keypair.json` is reused, **the program ID stays
`F7MQRA15…` — no backend/frontend rewrite**, only:

1. Buy ~**5 SOL** (~$400-700, mostly reclaimable program rent).
2. Run `solana program deploy --url mainnet` with the same keypair (same program ID).
   (Or re-run `deploy_sol_devnet.sh` logic against mainnet.)
3. Flip 3 config constants to mainnet:
   - `SOL_DEFAULT_NETWORK=mainnet` (backend env)
   - `REACT_APP_SOL_RPC_URL=https://api.mainnet-beta.solana.com` +
     `REACT_APP_SOL_DEVNET=false` (frontend env)
   - `UPL_DEPLOYED_SOL_JSON` → a `deployed_sol_mainnet.json` with `network:"mainnet"`
4. Redeploy backend + frontend. Badge auto-hides. Done.
